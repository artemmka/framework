...
import { Math3D, Ellipsoid, Cone, Cube, Sphere, Torus, Point, Edge, Polygon, EDistance, Surface, Light, Cylinder, ellipticalCylinder, ellipticalParaboloid, hyperboliccylinder, hyperbolicParaboloid, oneSheetedHyperboloid, paraboliccylinder, twoSheetedHyperboloid} from "../../modules/Math3D"
...
const Graph3D: React.FC = () => {
        ...
        if (graph) {
            ...
            if (custom.drawPolygons) {
                ...
                scene.forEach((surface: Surface, index: number) => {
                    ...
                    surface.polygons.forEach((polygon: Polygon,indexOfPoly: number) => {
                        polygon.indexOfPoly = indexOfPoly;
                        polygon.index = index;
                        polygons.push(polygon);
                    });
                });
                math3D.sortByArtistAlgorithm(polygons);
                polygons.forEach((polygon,index:number) => {
                    ...
                    if (graph){
                        ...
                        //graph.text((points[0].x+points[2].x)/2,(points[0].y+points[2].y)/2,polygon.indexOfPoly.toString(),'black',10);
                    }
                    }
                });
            }
            ...
            if (custom.drawPoints) {
                scene.forEach((surface: Surface) => {
                    surface.points.forEach((point: Point,index: number) => {
                        if (graph){
                            //graph.text(math3D.xs(point), math3D.ys(point),index.toString(),'black',12);
                            graph.point(math3D.xs(point), math3D.ys(point));}
                    });
                });
            }

            graph.text(WIN.left, WIN.bottom, fps.toString(),'black',20);
        }
    }
    ...
    const changeLumen = (event: React.ChangeEvent<HTMLInputElement>) => {
        ligth.lumen = Number(event.target.value);
    }
    useEffect(() => {
        graph = new Graph({
            id: canvasId,
    ...
    return (<div className="content">
        <canvas id={canvasId} className="cnvas"></canvas>
        <div>
            <Checkbox3D
                text="точки"
                id="points"
