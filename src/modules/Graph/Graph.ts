...
class Graph {
    ...
    polygon(points: Omit<Point, 'z'>[] = [], color: string = '#ff0000'): void {
        ...
        this.context.fill()
        this.context.closePath();
        this.text(this.xs(points[0].x),this.ys(points[0].y),'1488','black',1);
    }
    ...
    text = (x: number, y: number, text: string, color: string = "#000",pt: number): void => {
        this.context.font = `${pt}pt arial`;
        this.context.fillStyle = color;
